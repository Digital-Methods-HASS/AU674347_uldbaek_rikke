...
*The pfdtools have added these "\n", probably with the meaning of line shifts. Likewise has "SPM\" been added, not sure for what reason, again maybe shift of line* 
...
*As the pdf state them selves on page 6, the word "confidence" expresses a level of likelihood of an outcome i.e., either very low, low, medium, high and very high. This explains very well the over-usage of the word.*
...
### Loading in the data and convert into a dataframe
```{r}
...
### Extracting the GOT tokens 
```{r}
got_tokens <- got_df %>% 
  unnest_tokens(word, text_full)

#checking out amount of each token..lots of stopwords
got_wc <- got_tokens %>% 
  count(word) %>% 
  arrange(-n)
...
#checking out the amount of each token again
got_stopword_wc <- got_stopwords %>% 
  count(word) %>% 
  arrange(-n)

head(got_stopword_wc) #much better
```
...
### Plotting word cloud just for fun
```{r}
...
### Applying Sentiment analysis on the GOT text using the NRC lexicon

I will use "nrc" since it provides emotions.
```{r}
get_sentiments(lexicon = "nrc")

#combining GOT non-stopwords with nrc lexicon
got_nrc <- got_stopwords %>% 
  inner_join(get_sentiments("nrc"))

#checking out what words we just excluded 
...
head(got_exclude_n)
```
*I kinda find that lots of these words are pretty dominant and pretty important, both given the frequency count, but also visually from the cloud plot. This is problematic, however using the afinn lexicon instead does not cope with the problem, as it is also a predefined lexicon with similar properties.Therefore I continue using the NRC lexicon... * 


### Counting the by sentiment
```{r}
got_nrc_n <- got_nrc %>% 
  count(sentiment, sort = TRUE)
...
### okay, something is off with the word "lord"
...
lord
```
*Okay, so it seems like the word "lord" are present in the NRC lexicon in terms of "disgust", "negative", "positive", and "trust", which again points out the limitations of sentiment analysis by only using the predefined lexicons. It is hard to infer anything from this brief little sentiment analysis when such pitfalls are present.* 

### Answers to the questions of the sentiment analysis exercise
...
*The 5 most meaningful words are "lord", "king", "father", "bran", and "brother". And I'd expect a mixed set of emotions to dominate these 5 words which is the case.* 

#### 2) Are there any terms that are similarly ambiguous to the 'confidence' above?
*Yes, the word lord seems to be present in four different sentiment categories. This is very problematic.*
```{r}
lord
```
